...
which contains `R` functions that applies the proposed ensemble procedure to `ranger` objects (Wright and Ziegler, 2017).
```{R}
...
`getCON()` implements the concordance measure definded in Equation 6 of Sun et al. (2022)
and `getCON2()` implements a weighted concordance measure that assigns equal weights 
to all time points described at the end of Section 4 of Sun et al. (2022).
...
```
  ...
  - `sc` is a step function that presents an estimate of the censoring time survival function. 
  - `dat` is a data frame containing time and censoring indicator used to calculate the concordance measure.   
...
The `getSi()` function returns a list, where the $i$th member of the list gives the landmark probability 
