...
The `simDat3B()` function takes on two arguments, `n` and `cen` for sample size and censoring percentage at the baseline, respectively.
The `simDat3B()` function is also implemented in `codes/sim3B.R` but is included here for the completeness of this vignette.
...
The `getTrueSurv3B()` function from `codes/sim3B.R` compute the true landmark probability using 
...
The `getTrueSurv3B()` function returns a list, where the $i$th member of the list gives the true landmark probability for 
the $i$th subject from the testing data. 
...
The following implementation gives a survival prediction without the $i$th subject.
```{R}
...
```
The OOB concordance measure is then computed as follows. 
```{R}
con0 <- mean(sapply(t0, getCON, getSi(fit, dat), sc, dat), na.rm = T)
```
The following generates 50 permutations for each predictor $Z_1, \ldots, Z_{10}, W_1(U), \ldots, W_{10}(U)$ 
